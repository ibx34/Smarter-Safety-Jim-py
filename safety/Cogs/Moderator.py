...
class Moderator(Plugin):
    ...
    async def lock(self,ctx,channel:discord.TextChannel=None):
        ...
        if self.bot.locked_channels.get(f"{ctx.guild.id}-{channel.id}"):
            return await ctx.no(f"There is already saved perms for {channel.mention}. Please run `%unlock {channel.id}` to remove it.")

        self.bot.locked_channels[f"{ctx.guild.id}-{channel.id}"] = channel.overwrites_for(ctx.guild.default_role)
        await channel.set_permissions(ctx.guild.default_role,send_messages=False,add_reactions=True)

        await ctx.yes(f"Locked {channel.mention}.")

    @commands.command(name="unlocK",aliases=["uncease","unkillchat","start","continue"])
    @commands.guild_only()
    @commands.has_permissions(manage_channels=True)
    ...
    async def globalban(self,ctx,user:discord.Member,*,reason:Reason=None):
        ...
        confirm = await prompt(self=ctx,message=dedent(f"""{user} will be banned in {"over" if len(self.bot.guilds) > 1 else ""} {len(self.bot.guilds)} guilds."""),author_id=ctx.author.id,reacquire=False)
        
        if not confirm:
            return await ctx.send(f"Alright, **{user}** hasn't been banned. If you would like to ban them **in this server** use the normal ban command.")
        ...
        embed.add_field(name="Server",value=f"**{ctx.guild}** (`{ctx.guild.id}`)",inline=False)
        ...
        async with ctx.channel.typing():
            for x in self.bot.guilds:
                try:
                    await x.ban(user,delete_message_days=7,reason=f"[GLOBAL BAN]: {reason}")
                except:
                    pass
        await ctx.yes(dedent(f"""**User**: {user} has been banned in {"over" if len(self.bot.guilds) > 1 else ""} {len(self.bot.guilds)} guilds."""))
